...
{
    LOGCATD("NativeRender::Init m_NativeWindow=%p, video[w,h]=[%d, %d]",
            m_NativeWindow, videoWidth, videoHeight);
    if(m_NativeWindow == nullptr)
    {
        LOGCATD("NativeRender::Init return 1");
        return;
    }
    ...
    LOGCATD("NativeRender::Init window[w,h]=[%d, %d],DstSize[w, h]=[%d, %d]",
            windowWidth, windowHeight, m_DstWidth, m_DstHeight);
...
void NativeRender::RenderVideoFrame(NativeImage *pImage)
{
    ...
    LOGCATD("NativeRender::RenderVideoFrame srcLineSize = %d, dstLineSize = %d ",
            srcLineSize,dstLineSize);
    for (int i = 0; i < m_DstHeight; ++i) {
        //将数组src复制到数组dst，到底拷贝了多少？？
        //是根据屏幕一行一行拷贝？？
        memcpy(dstBuffer + i * dstLineSize,
                pImage->ppPlane[0] + i * srcLineSize, srcLineSize);
    }
