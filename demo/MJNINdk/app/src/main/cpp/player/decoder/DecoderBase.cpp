...
int DecoderBase::InitFFDecoder()
{
    ...
    do
    {
        ...
        //9,//创建 AVPacket 存放编码数据
        m_Duration = m_AVFormatContext->duration;
        m_Packet = av_packet_alloc();
        //10,创建 AVFrame 存放解码后的数据
        m_Frame = av_frame_alloc();
        LOGCATD("DecoderBase::InitFFDecoder  success!!!");
    } while (false);
...
void DecoderBase::UnInit() {
    ...
    if(m_Thread) {
         ...
         *
         *  一般都是在析构函数中调用，通过delete释放对象，会执行到这里
         *
         */
        m_Thread->join();
        delete m_Thread;
...
{
    LOGCATD("DecoderBase::DecodingLoop start, m_MediaType=%d", m_MediaType);
    std::unique_lock<std::mutex> lock(m_Mutex);
    m_DecoderState = STATE_DECODING;
    lock.unlock();
    for(;;)
    ...
    LOGCATD("DecoderBase::DecodingLoop end");
}

void DecoderBase::UpdateTimeStamp()
{
    LOGCATD("DecoderBase::UpdateTimeStamp");
    std::unique_lock<std::mutex> lock(m_Mutex);
    ...
    m_CurTimeStamp = (int64_t)((m_CurTimeStamp *
